...
Everything the application serves is committed — the scored assets, the action briefs, the metrics
and the cached LLM results. **Nothing needs to be built, and no API key is required.**
...
Then open <http://127.0.0.1:8000>. Python 3.11 or later; no database, no external services.
...
The application reads the JSON in `output/` once at startup and serves it. There is no inference, no
network call and no external asset at request time; its only write is appending to
...
Tests read the committed `output/` files, so they also run without rebuilding anything.
...
## Appendix — rebuilding the pipeline

Not required to run or assess the prototype. This regenerates every committed artefact from the seed,
which takes a few minutes and reproduces the current `data/` and `output/` byte for byte.

Run the five scripts in order. Each is independently re-runnable and idempotent.
...
### Rebuilding with no API key

`extract.py` and `retrieve.py` are the only scripts that call an LLM, and every call is cached to
disk by a hash of its prompt. The cache directories are committed, so both complete offline:
